...
# Check and print null counts column-wise
duplicates_count = df.duplicated().sum()
print(f"\nNumber of duplicate rows in the dataset: {duplicates_count}")
print("\nColumn-wise null value count:")
print(df.isnull().sum())

# Get the number of rows and columns in the dataset and print it
rows, cols = df.shape
print(f"\nThe dataset contains {rows} rows and {cols} columns.")

# If 'price' column exists, filter and display rows where the 'price' is greater than 30
...
# If 'area' column exists, fill the missing values in the 'area' column with its mean
